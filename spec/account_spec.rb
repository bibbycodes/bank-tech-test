...
describe Account do
  let(:deposit) do
    double('Transaction', date: '10/10/2012', amount: 1000, type: 'deposit')
  end
  let(:withdrawal) do
    double('Transaction', date: '02/10/2012', amount: 2000, type: 'withdrawal')
  end
  ...
  context '#balance' do
    ...
    it 'returns 2000 when 2000 has been withdrawn from an account with 4000 inside' do
      ...
      expect(@full_account.balance).to eq(2000)
    end
  end
  ...
  context '#ledger' do
    ...
    it 'returns [deposit, withdrawal] when a deposit followed by a withdrawal has been made' do
      ...
      expect(@account.ledger).to eq([deposit, withdrawal])
    end
  end
end
